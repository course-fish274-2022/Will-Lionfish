...
test <- surveys %>% 
  ...
  mutate(habitat_names = case_when(
  habitat == 1  ~ "mixed",
  habitat == 2  ~ "high-profile reef",
  habitat == 3  ~ "low-profile  reef",
  habitat == 4  ~ "sloping dropoff",))


  ggplot(test, aes(x = year, y = abundance))+
  geom_histogram(stat = 'identity') +
  facet_wrap(~habitat_names)+  
